...
if (isset($_POST['submit'])) {
    ...
    if (isset($_POST['id']) && $_POST['id']) {
        ...
        if ($result !== false) {
            echo '<div class="notice notice-success"><p>' . esc_html__('✅ Extra erfolgreich aktualisiert!', 'h2-rental-pro') . '</p></div>';
            if ($existing) {
        ...
        if ($result !== false) {
            ...
            echo '<div class="notice notice-success"><p>' . esc_html__('✅ Extra erfolgreich hinzugefügt!', 'h2-rental-pro') . '</p></div>';
            $res = \ProduktVerleih\StripeService::create_extra_price($extra_base_name, $stripe_price, $main_product_name, $modus);
            if (is_wp_error($res)) {
                // handle Stripe error silently
            } elseif (!empty($res['price_id'])) {
...
if (isset($_GET['delete']) && isset($_GET['fw_nonce']) && wp_verify_nonce($_GET['fw_nonce'], 'produkt_admin_action')) {
    ...
    if ($result !== false) {
        echo '<div class="notice notice-success"><p>' . esc_html__('✅ Extra gelöscht!', 'h2-rental-pro') . '</p></div>';
    } else {
...
$subline_text = esc_html__('Extras verwalten', 'h2-rental-pro');
if ($active_tab === 'add') {
    $subline_text = sprintf(esc_html__('Erstellen Sie ein neues Extra für das Produkt "%s"', 'h2-rental-pro'), ($current_category ? esc_html($current_category->name) : esc_html__('Unbekannt', 'h2-rental-pro')));
} elseif ($active_tab === 'edit' && $edit_item) {
    $subline_text = sprintf(esc_html__('Bearbeiten Sie das Extra "%s" für das Produkt "%s"', 'h2-rental-pro'), esc_html($edit_item->name), ($current_category ? esc_html($current_category->name) : esc_html__('Unbekannt', 'h2-rental-pro')));
}
?>
            ...
            <div class="dashboard-left">
                <div class="dashboard-card card-product-selector">
                    ...
                    <p class="card-subline">
                        <?php echo esc_html__('Für welches Produkt möchten Sie ein Extra bearbeiten?', 'h2-rental-pro'); ?>
                    </p>
            ...
            <div class="dashboard-right">
                <div class="dashboard-row">
                    <div class="dashboard-card card-new-product">
                        <h2><?php echo esc_html__('Neues Extra', 'h2-rental-pro'); ?></h2>
                        <p class="card-subline"><?php echo esc_html__('Extra erstellen', 'h2-rental-pro'); ?></p>
                ...
                <div class="dashboard-card">
                    <div class="card-header-flex">
                        <div>
                            ...
                            <p class="card-subline">
                                <?php echo esc_html__('Verfügbare Extras des Produkts', 'h2-rental-pro'); ?></p>
                        </div>
                    </div>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <button type="button" class="icon-btn"
                                            ...
                                            onclick="window.location.href='?page=produkt-extras&category=<?php echo $selected_category; ?>&tab=edit&edit=<?php echo $extra->id; ?>'">
                                        ...
                                        <button type="button" class="icon-btn"
                                            onclick="if(confirm('<?php echo esc_js(__('Bist du sicher das du Löschen möchtest?', 'h2-rental-pro')); ?>')){window.location.href='?page=produkt-extras&category=<?php echo $selected_category; ?>&delete=<?php echo $extra->id; ?>&fw_nonce=<?php echo wp_create_nonce('produkt_admin_action'); ?>';}"
