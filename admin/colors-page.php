...
<div class="produkt-admin dashboard-wrapper">
    ...
    <p class="dashboard-subline"><?php echo esc_html__('Farben verwalten', 'h2-rental-pro'); ?></p>
        ...
        <div class="dashboard-left">
            <div class="dashboard-card card-product-selector">
                ...
                <p class="card-subline">
                    <?php echo esc_html__('Für welches Produkt möchten Sie Farben bearbeiten?', 'h2-rental-pro'); ?>
                </p>
        ...
        <div class="dashboard-right">
            <div class="dashboard-row">
                <div class="dashboard-card card-new-product">
                    <h2><?php echo esc_html__('Neue Farbe', 'h2-rental-pro'); ?></h2>
                    <p class="card-subline"><?php echo esc_html__('Farbe hinzufügen', 'h2-rental-pro'); ?></p>
                    <a href="#" id="add-color-btn" class="icon-btn add-product-btn"
                            ...
                            <a href="admin.php?page=produkt-variants">
                                <div class="quicknav-inner">
                                    ...
                                    <div class="quicknav-label">
                                        <?php echo esc_html__('Ausführungen', 'h2-rental-pro'); ?>
                                    </div>
                                </div>
                            </a>
                        </div>
            ...
            <div class="dashboard-card">
                <div class="card-header-flex">
                    <div>
                        <h2><?php echo esc_html__('Produktfarben', 'h2-rental-pro'); ?></h2>
                        <p class="card-subline">
                            <?php echo esc_html__('Vorhandene Farben des Produkts', 'h2-rental-pro'); ?>
                        </p>
                    </div>
                </div>
                <table class="activity-table">
                    <thead>
                        <tr>
                            <th><?php echo esc_html__('Farbname', 'h2-rental-pro'); ?></th>
                            <th><?php echo esc_html__('Farbcode', 'h2-rental-pro'); ?></th>
                            <th><?php echo esc_html__('Verfügbarkeit je Ausführung', 'h2-rental-pro'); ?></th>
                    ...
                    <tbody>
                        <?php if (empty($product_colors)): ?>
                            <tr>
                                <td colspan="4"><?php echo esc_html__('Keine Farben vorhanden.', 'h2-rental-pro'); ?></td>
                            </tr>
                        <?php else:
                            foreach ($product_colors as $color):
                                ...
                                <tr>
                                    ...
                                    <td><span class="<?php echo esc_attr($preview_class); ?>"
                                            style="<?php echo esc_attr($preview_style); ?>"></span>
                                        <?php echo empty($color->is_multicolor) ? esc_html($color->color_code) : esc_html__('Mehrfarbig', 'h2-rental-pro'); ?>
                                    </td>
                                    <td><?php echo esc_html($availability); ?></td>
                                    <td>
                                        <button type="button" class="icon-btn"
                                            aria-label="<?php echo esc_attr__('Bearbeiten', 'h2-rental-pro'); ?>"
                                            onclick="window.location.href='<?php echo admin_url('admin.php?page=produkt-colors&category=' . $selected_category . '&tab=edit&edit=' . $color->id); ?>'">
                                        ...
                                        <button type="button" class="icon-btn"
                                            onclick="if(confirm('<?php echo esc_js(__('Bist du sicher das du Löschen möchtest?', 'h2-rental-pro')); ?>')){window.location.href='<?php echo admin_url('admin.php?page=produkt-colors&category=' . $selected_category . '&tab=list&delete=' . $color->id . '&produkt_admin_nonce=' . wp_create_nonce('produkt_admin_action')); ?>';}"
            ...
            <?php if (!$is_sale): ?>
                <div class="dashboard-card">
                    <div class="card-header-flex">
                        <div>
                            <h2><?php echo esc_html__('Gestellfarben', 'h2-rental-pro'); ?></h2>
                            <p class="card-subline"><?php echo esc_html__('Vorhandene Gestellfarben', 'h2-rental-pro'); ?>
                            </p>
                        </div>
                    </div>
                    <table class="activity-table">
                        ...
                        <tbody>
                            <?php if (empty($frame_colors)): ?>
                                <tr>
                                    <td colspan="4"><?php echo esc_html__('Keine Gestellfarben vorhanden.', 'h2-rental-pro'); ?>
                                    </td>
                                </tr>
                            <?php else:
    ...
    <div id="color-modal" class="modal-overlay" data-open="<?php echo ($active_tab !== 'list') ? '1' : '0'; ?>">
        <div class="modal-content">
            ...
            <h2><?php echo $active_tab === 'edit' ? esc_html__('Farbe bearbeiten', 'h2-rental-pro') : esc_html__('Neue Farbe', 'h2-rental-pro'); ?>
            </h2>
            <form method="post" class="produkt-compact-form">
                ...
                <div class="produkt-form-group">
                    <label for="color-name"><?php echo esc_html__('Farbname', 'h2-rental-pro'); ?></label>
                ...
                ?>
                <div class="produkt-form-group produkt-color-code-group"
                    style="<?php echo $is_multicolor ? 'display:none;' : ''; ?>">
                    <label for="color-code"><?php echo esc_html__('Farbcode', 'h2-rental-pro'); ?></label>
                    <div class="produkt-color-picker">
                        <div class="<?php echo esc_attr($preview_class); ?>"
                            style="<?php echo esc_attr($preview_style); ?>"></div>
                        <input type="text" class="produkt-color-value" name="color_code"
                            value="<?php echo esc_attr($edit_item->color_code ?? '#ffffff'); ?>">
                        <input type="color" class="produkt-color-input"
                            value="<?php echo esc_attr($edit_item->color_code ?? '#ffffff'); ?>">
                    </div>
                </div>
                <div class="produkt-form-group">
                    <label class="produkt-toggle-label" for="color-multicolor" style="gap:0.5rem;">
                        ...
                        <span><?php echo esc_html__('Mehrfarbig', 'h2-rental-pro'); ?></span>
                    </label>
                </div>
                <?php if (!$is_sale): ?>
                    <div class="produkt-form-group">
                        <label for="color-type"><?php echo esc_html__('Typ', 'h2-rental-pro'); ?></label>
                        <select name="color_type" id="color-type">
                            <option value="product" <?php selected($edit_item->color_type ?? '', 'product'); ?>>
                                <?php echo esc_html__('Produktfarbe', 'h2-rental-pro'); ?>
                            </option>
                            <option value="frame" <?php selected($edit_item->color_type ?? '', 'frame'); ?>>
                                <?php echo esc_html__('Gestellfarbe', 'h2-rental-pro'); ?>
                            </option>
                        </select>
                    </div>
                <?php else: ?>
                ...
                <div class="produkt-form-group">
                    ...
                    <div class="image-field-row">
                        <div class="image-preview"
                            style="<?php echo !empty($edit_item->image_url) ? 'background-image:url(' . esc_url($edit_item->image_url) . ');' : ''; ?>">
                        </div>
                        <input type="hidden" name="image_url"
                            value="<?php echo esc_url($edit_item->image_url ?? ''); ?>">
                        <button type="button" class="icon-btn image-select"
                            aria-label="<?php echo esc_attr__('Bild auswählen', 'h2-rental-pro'); ?>">
                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 82.3 82.6">
                                <path
