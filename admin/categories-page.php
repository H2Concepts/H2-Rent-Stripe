...
$subline_text = __('Produkte verwalten', 'h2-rental-pro');
if ($active_tab === 'add') {
    $subline_text = __('Erstellen Sie eine Produkt und Produktseite mit individuellen Einstellungen und Konfigurationen.', 'h2-rental-pro');
} elseif ($active_tab === 'edit' && $edit_item) {
    $subline_text = sprintf(__('Bearbeiten Sie das Produkt "%s" mit allen Einstellungen und Inhalten.', 'h2-rental-pro'), esc_html($edit_item->name));
}
?>
        ...
        <div class="dashboard-grid">
            <div class="dashboard-left">
                <div class="dashboard-card">
                    <h2><?php echo esc_html__('Neueste Produkte', 'h2-rental-pro'); ?></h2>
                    <p class="card-subline"><?php echo esc_html__('Die zuletzt hinzugefügten Produkte', 'h2-rental-pro'); ?>
                    </p>
                    <div class="recent-product-tiles">
                        <?php foreach ($recent_products as $prod): ?>
                            <?php $has_image = !empty($prod->default_image); ?>
                            <div class="recent-product-tile<?php echo $has_image ? '' : ' no-image'; ?>" <?php echo $has_image ? ' style="background-image:url(' . esc_url($prod->default_image) . ')"' : ''; ?>
                                onclick="window.location.href='?page=produkt-categories&tab=edit&edit=<?php echo $prod->id; ?>'">
                                <?php if (!$has_image): ?>
                                ...
                                <div class="tile-overlay">
                                    ...
                                    <button type="button" class="icon-btn edit-btn"
                                        ...
                                        onclick="event.stopPropagation();window.location.href='?page=produkt-categories&tab=edit&edit=<?php echo $prod->id; ?>'">
            ...
            <div class="dashboard-right">
                <div class="dashboard-row">
                    <div class="dashboard-card card-new-product">
                        <h2><?php echo esc_html__('Neues Produkt', 'h2-rental-pro'); ?></h2>
                        <p class="card-subline"><?php echo esc_html__('Produkt erstellen', 'h2-rental-pro'); ?></p>
                        ...
                        <div class="quicknav-grid">
                            ...
                            <?php if (!$is_sale): ?>
                                <div class="quicknav-card">
                                    <a href="admin.php?page=produkt-conditions">
                                        <div class="quicknav-inner">
                                            <div class="quicknav-icon-circle">🔧</div>
                                            <div class="quicknav-label"><?php echo esc_html__('Zustand', 'h2-rental-pro'); ?>
                                            </div>
                                        </div>
                                    </a>
                                </div>
                            ...
                            <div class="quicknav-card">
                                <a href="admin.php?page=produkt-colors">
                                    <div class="quicknav-inner">
                                        <div class="quicknav-icon-circle">🎨</div>
                                        <div class="quicknav-label"><?php echo esc_html__('Farben', 'h2-rental-pro'); ?>
                                        </div>
                                    </div>
                                </a>
                            </div>
                ...
                <div class="dashboard-card">
                    <div class="card-header-flex">
                        <div>
                            <h2><?php echo esc_html__('Alle Produkte', 'h2-rental-pro'); ?></h2>
                            <p class="card-subline">
                                <?php echo esc_html__('Produkte nach Kategorien anzeigen lassen', 'h2-rental-pro'); ?></p>
                        </div>
                        <form method="get" class="produkt-filter-form product-search-bar">
                            ...
                            <div class="search-input-wrapper">
                                ...
                                <input type="text" name="s"
                                    placeholder="<?php echo esc_attr__('Suchen nach Produkten', 'h2-rental-pro'); ?>"
                                    value="<?php echo esc_attr($search_term); ?>">
                            </div>
                            <select name="prodcat">
                                <option value="0"><?php echo esc_html__('Alle Kategorien', 'h2-rental-pro'); ?></option>
                                <?php foreach ($product_categories as $pc): ?>
                                    <option value="<?php echo $pc->id; ?>" <?php selected($selected_prodcat, $pc->id); ?>>
                                        <?php echo str_repeat('--', $pc->depth) . ' ' . esc_html($pc->name); ?></option>
                                <?php endforeach; ?>
                            </select>
                    ...
                    <table class="activity-table">
                        <thead>
                            <tr>
                                ...
                                <th><?php echo esc_html__('Kategorien', 'h2-rental-pro'); ?></th>
                                <th><?php echo esc_html__('SEO', 'h2-rental-pro'); ?></th>
                                ...
                                <tr>
                                    <td>
                                        <?php if (!empty($cat->default_image)): ?>
                                            <img src="<?php echo esc_url($cat->default_image); ?>"
                                    ...
                                    <td>
                                        <button type="button" class="icon-btn"
                                            aria-label="<?php echo esc_attr__('Bearbeiten', 'h2-rental-pro'); ?>"
                                            onclick="window.location.href='?page=produkt-categories&tab=edit&edit=<?php echo $cat->id; ?>'">
                                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 80.8 80.1">
                                                <path
                                                    d="M54.7,4.8l-31.5,31.7c-.6.6-1,1.5-1.2,2.3l-3.3,18.3c-.2,1.2.2,2.7,1.2,3.8.8.8,1.9,1.2,2.9,1.2h.8l18.3-3.3c.8-.2,1.7-.6,2.3-1.2l31.7-31.7c5.8-5.8,5.8-15.2,0-21-6-5.8-15.4-5.8-21.2,0h0ZM69.9,19.8l-30.8,30.8-11,1.9,2.1-11.2,30.6-30.6c2.5-2.5,6.7-2.5,9.2,0,2.5,2.7,2.5,6.7,0,9.2Z" />
                                                <path
                                        ...
                                        <button type="button" class="icon-btn"
                                            onclick="if(confirm('<?php echo esc_js(__('Bist du sicher das du Löschen möchtest?', 'h2-rental-pro')); ?>')){window.location.href='?page=produkt-categories&delete=<?php echo $cat->id; ?>&fw_nonce=<?php echo wp_create_nonce('produkt_admin_action'); ?>';}"
