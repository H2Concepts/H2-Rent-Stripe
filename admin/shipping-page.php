    ...
    function produkt_create_stripe_shipping_item($name, $price)
    {
        ...
        try {
            $product = \Stripe\Product::create([
                'name' => __('Versand: ', 'h2-rental-pro') . $name,
...
if (isset($_POST['save_shipping'])) {
    if (
        ...
        wp_die(__('Ungültige Anfrage – bitte Seite neu laden und erneut versuchen.', 'h2-rental-pro'));
    }
...
if (isset($_POST['save_free_shipping'])) {
    ...
    echo '<div class="notice notice-success"><p>' . esc_html__('✅ Kostenloser Versand aktualisiert!', 'h2-rental-pro') . '</p></div>';
}

// Handle delete
if (
    isset($_GET['delete']) && isset($_GET['fw_nonce']) &&
    ...
    if (current_user_can('manage_options')) {
        ...
        if ($result !== false) {
            echo '<div class="notice notice-success"><p>' . esc_html__('✅ Versandart gelöscht!', 'h2-rental-pro') . '</p></div>';
        } else {
...
$providers = [
    'none' => __('Ohne', 'h2-rental-pro'),
    'pickup' => __('Abholung', 'h2-rental-pro'),
    'dhl' => 'DHL',
    'hermes' => 'Hermes',
    'ups' => 'UPS',
    'dpd' => 'DPD'
...
<div class="produkt-admin dashboard-wrapper">
    <div id="shipping-modal" class="modal-overlay" data-open="<?php echo $edit_shipping ? '1' : '0'; ?>">
        <div class="modal-content">
            ...
            <h2><?php echo $edit_shipping ? esc_html__('Versandart bearbeiten', 'h2-rental-pro') : esc_html__('Neue Versandart hinzufügen', 'h2-rental-pro'); ?>
            </h2>
            <form method="post" class="produkt-compact-form">
                ...
                <div class="form-grid">
                    <div class="produkt-form-group">
                        <label for="shipping_name"><?php echo esc_html__('Name', 'h2-rental-pro'); ?></label>
                        <input type="text" id="shipping_name" name="shipping_name" required
                            value="<?php echo esc_attr($edit_shipping->name ?? ''); ?>">
                    </div>
                    <div class="produkt-form-group">
                        <label
                            for="shipping_price"><?php echo esc_html__('Versandkosten (€)', 'h2-rental-pro'); ?></label>
                        <input type="number" id="shipping_price" name="shipping_price" step="0.01" required
                            value="<?php echo esc_attr($edit_shipping->price ?? ''); ?>">
                    </div>
                    <div class="produkt-form-group">
                        <label
                            for="shipping_provider"><?php echo esc_html__('Dienstleister', 'h2-rental-pro'); ?></label>
                    ...
                    <div class="produkt-form-group full-width">
                        <label
                            for="shipping_description"><?php echo esc_html__('Beschreibung', 'h2-rental-pro'); ?></label>
                        <textarea id="shipping_description" name="shipping_description"
                            rows="3"><?php echo esc_textarea($edit_shipping->description ?? ''); ?></textarea>
                    </div>
                </div>
                <p>
                    <button type="submit" name="save_shipping" class="icon-btn"
    ...
    <p class="dashboard-subline"><?php echo esc_html__('Versandarten verwalten', 'h2-rental-pro'); ?></p>

    <div class="h2-rental-card card-shipping-list">
        <div class="card-header-flex">
            <div>
                <h2><?php echo esc_html__('Versandarten', 'h2-rental-pro'); ?></h2>
                <p class="card-subline"><?php echo esc_html__('Lieferoptionen für Bestellungen', 'h2-rental-pro'); ?>
                </p>
            </div>
            <button id="add-shipping-btn" type="button" class="icon-btn add-category-btn"
        ...
        <table class="activity-table">
            <thead>
                <tr>
                    ...
                    <th><?php echo esc_html__('Dienstleister', 'h2-rental-pro'); ?></th>
                    <th><?php echo esc_html__('Standard', 'h2-rental-pro'); ?></th>
                    ...
                    <tr>
                        ...
                        <td>
                            <button type="button" class="icon-btn"
                                ...
                                onclick="window.location.href='?page=produkt-shipping&edit=<?php echo $r->id; ?>'">
                            ...
                            <button type="button" class="icon-btn"
                                ...
                                onclick="if(confirm('<?php echo esc_js(__('Bist du sicher das du Löschen möchtest?', 'h2-rental-pro')); ?>')){window.location.href='?page=produkt-shipping&delete=<?php echo $r->id; ?>&fw_nonce=<?php echo wp_create_nonce('produkt_admin_action'); ?>';}">
    ...
    <form method="post" class="h2-rental-card produkt-compact-form">
        ...
        <div class="card-header-flex">
            <div>
                <h2><?php echo esc_html__('Kostenloser Versand', 'h2-rental-pro'); ?></h2>
                <p class="card-subline">
                    <?php echo esc_html__('Versandkosten automatisch ab einem Warenwert erlassen', 'h2-rental-pro'); ?>
                </p>
            </div>
        ...
        <div class="form-grid">
            <div class="produkt-form-group">
                <label
                    for="free_shipping_threshold"><?php echo esc_html__('Kostenloser Versand ab', 'h2-rental-pro'); ?></label>
                <div class="input-with-suffix">
                    <input type="number" step="0.01" min="0" id="free_shipping_threshold" name="free_shipping_threshold"
                        value="<?php echo esc_attr($free_shipping_threshold); ?>" placeholder="59"
                        data-free-shipping-threshold>
                    <span class="suffix">€</span>
                </div>
                <p class="description">
                    <?php echo esc_html__('Warenwert, ab dem im Checkout keine Versandkosten mehr berechnet werden.', 'h2-rental-pro'); ?>
                </p>
            </div>
        </div>
        <p>
            <button type="submit" class="icon-btn" aria-label="<?php echo esc_attr__('Speichern', 'h2-rental-pro'); ?>">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 80.3 80.3">
                    <path
                        d="M32,53.4c.8.8,1.9,1.2,2.9,1.2s2.1-.4,2.9-1.2l20.8-20.8c1.7-1.7,1.7-4.2,0-5.8-1.7-1.7-4.2-1.7-5.8,0l-17.9,17.9-7.7-7.7c-1.7-1.7-4.2-1.7-5.8,0-1.7,1.7-1.7,4.2,0,5.8l10.6,10.6Z" />
                    <path
