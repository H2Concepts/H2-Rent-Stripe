...
<div class="produkt-edit-duration">
    <div class="produkt-form-header">
        <h3><?php echo esc_html__('✏️ Mietdauer bearbeiten', 'h2-rental-pro'); ?></h3>
        <p><?php printf(esc_html__('Bearbeiten Sie die Mietdauer "%s" für das Produkt "%s"', 'h2-rental-pro'), esc_html($edit_item->name), $current_category ? esc_html($current_category->name) : esc_html__('Unbekannt', 'h2-rental-pro')); ?>
        </p>
    </div>
    ...
    <form method="post" action="" class="produkt-compact-form">
        ...
        <button type="submit" name="submit" class="icon-btn durations-save-btn"
        ...
        <a href="<?php echo admin_url('admin.php?page=produkt-durations&category=' . $selected_category . '&delete=' . $edit_item->id . '&fw_nonce=' . wp_create_nonce('produkt_admin_action')); ?>"
            class="icon-btn durations-delete-btn" aria-label="<?php echo esc_attr__('Löschen', 'h2-rental-pro'); ?>"
            onclick="return confirm('<?php echo esc_js(sprintf(__('Sind Sie sicher, dass Sie diese Mietdauer löschen möchten?\n\n"%s" wird unwiderruflich gelöscht!', 'h2-rental-pro'), $edit_item->name)); ?>')">
        ...
        <div class="produkt-form-section">
            ...
            <div class="produkt-form-row">
                ...
                <div class="produkt-form-group">
                    ...
                    <input type="number" name="months_minimum" value="<?php echo $edit_item->months_minimum; ?>" min="1"
                        required>
                </div>
            </div>
        ...
        <div class="produkt-form-section">
            ...
            <?php foreach ($variants as $variant): ?>
                <div class="produkt-form-group">
                    ...
                    <input type="number" step="0.01" name="variant_custom_price[<?php echo $variant->id; ?>]"
                        value="<?php echo esc_attr($duration_prices[$variant->id]['custom_price'] ?? ''); ?>">
                    ...
                    <?php if ($archived): ?>
                        <span
                            class="badge badge-gray"><?php echo esc_html__('Archivierter Stripe-Preis', 'h2-rental-pro'); ?></span>
                    <?php endif; ?>
