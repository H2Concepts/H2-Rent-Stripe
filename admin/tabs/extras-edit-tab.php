...
<div class="produkt-edit-extra">
    <form method="post" action="" class="produkt-compact-form">
        ...
        <button type="submit" name="submit" class="icon-btn extras-save-btn"
        ...
        <a href="<?php echo admin_url('admin.php?page=produkt-extras&category=' . $selected_category . '&delete=' . $edit_item->id . '&fw_nonce=' . wp_create_nonce('produkt_admin_action')); ?>"
            class="icon-btn extras-delete-btn" aria-label="<?php echo esc_attr__('Löschen', 'h2-rental-pro'); ?>"
        ...
        <div class="produkt-form-sections">
            ...
            <div class="dashboard-card">
                ...
                <p class="card-subline"><?php echo esc_html__('Name', 'h2-rental-pro'); ?></p>
                <div class="form-grid">
            ...
            <div class="dashboard-card">
                <h2><?php echo esc_html__('Preis', 'h2-rental-pro'); ?></h2>
                <p class="card-subline"><?php echo esc_html__('Tarif', 'h2-rental-pro'); ?></p>
                    ...
                    <div class="price-card" data-field="<?php echo $modus === 'kauf' ? 'sale_price' : 'price'; ?>">
                        ...
                        <div class="price-display">
                            <input type="text" class="price-input" placeholder="0,00"
                                aria-label="<?php echo esc_attr__('Preis in Euro', 'h2-rental-pro'); ?>" />
                            <span class="price-suffix">€</span>
                            <input type="hidden" name="<?php echo $modus === 'kauf' ? 'sale_price' : 'price'; ?>"
                                class="price-hidden"
                                value="<?php echo $modus === 'kauf' ? intval(round($sale_price * 100)) : intval(round($edit_item->price * 100)); ?>">
                        </div>
            ...
            <div class="dashboard-card">
                <h2><?php echo esc_html__('Extra-Bild', 'h2-rental-pro'); ?></h2>
                <p class="card-subline"><?php echo esc_html__('Vorschau', 'h2-rental-pro'); ?></p>
                <div class="form-grid">
                    <div class="produkt-form-group full-width">
                        <label><?php echo esc_html__('Extra-Bild', 'h2-rental-pro'); ?></label>
                        <div class="image-field-row">
                            ...
                            <button type="button" class="icon-btn produkt-remove-image" data-target="image_url"
                        ...
                        <input type="hidden" name="image_url" id="image_url"
                            value="<?php echo esc_attr($edit_item->image_url ?? ''); ?>">
                        <small><?php echo esc_html__('Wird als Overlay über dem Hauptbild angezeigt (empfohlen: 400x400 Pixel)', 'h2-rental-pro'); ?></small>
                    </div>
                </div>
            </div>
            ...
            <?php if (!empty($variants)): ?>
                <div class="dashboard-card">
                    <h2><?php echo esc_html__('Verfügbarkeit je Ausführung', 'h2-rental-pro'); ?></h2>
                    <p class="card-subline"><?php echo esc_html__('Extras pro Variante aktivieren', 'h2-rental-pro'); ?></p>
                        ...
                        <?php foreach ($variants as $v): ?>
                            ...
                            <div class="variant-availability-card">
                                ...
                                <div class="availability-prices">
                                    ...
                                    <?php if ($std_price > 0): ?>
                                        <div>
                                            <?php printf(esc_html__('Standardpreis: %s€', 'h2-rental-pro'), number_format($std_price, 2, ',', '.')); ?>
                                        </div>
                                    <?php endif; ?>
                                    <?php if ($modus === 'kauf' && !empty($v->weekend_price)): ?>
                                        <div>
                                            <?php printf(esc_html__('Wochenendpreis: %s€', 'h2-rental-pro'), number_format($v->weekend_price, 2, ',', '.')); ?>
                                        </div>
                                    <?php endif; ?>
                                </div>
                            </div>
