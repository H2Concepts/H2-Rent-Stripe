    ...
    <table class="activity-table">
        <thead>
            <tr>
                ...
                <th><?php echo esc_html__('Mindestlaufzeit', 'h2-rental-pro'); ?></th>
                <th><?php echo esc_html__('Preis', 'h2-rental-pro'); ?></th>
                <th><?php echo esc_html__('Badges', 'h2-rental-pro'); ?></th>
            ...
            <?php foreach ($durations as $duration):
                ...
                <tr>
                    <td>
                        ...
                        <?php if ($is_archived): ?>
                            <span
                                class="badge badge-warning"><?php echo esc_html__('Archivierter oder ungültiger Stripe-Preis', 'h2-rental-pro'); ?></span>
                        <?php endif; ?>
                        <?php if ($product_archived): ?>
                            <span
                                class="badge badge-danger"><?php echo esc_html__('⚠️ Produkt bei Stripe archiviert', 'h2-rental-pro'); ?></span>
                        <?php endif; ?>
                    </td>
                    <td>
                        <?php echo sprintf(
                            /* translators: %d: number of months */
                            _n('%d Monat', '%d Monate', intval($duration->months_minimum), 'h2-rental-pro'),
                            intval($duration->months_minimum)
                        ); ?>
                    </td>
                    <td>
                        <?php if (!is_null($duration->monthly_price)): ?>
                            <?php echo number_format(floatval($duration->monthly_price), 2, ',', '.'); ?>€
                        <?php else: ?>
                            –
                        <?php endif; ?>
                    </td>
                        ...
                        <?php if ($duration->show_badge || !empty($duration->show_popular)): ?>
                            <div class="produkt-duration-badges">
                                <?php if ($duration->show_badge): ?>
                                    <span
                                        class="produkt-discount-badge"><?php echo esc_html__('Rabatt-Badge', 'h2-rental-pro'); ?></span>
                                <?php endif; ?>
                                <?php if (!empty($duration->show_popular)): ?>
                                    <span class="badge badge-popular"
                                        style="<?php echo esc_attr($popular_style); ?>"><?php echo esc_html__('beliebt', 'h2-rental-pro'); ?></span>
                                <?php endif; ?>
                            </div>
                        <?php else: ?>
                            –
                    ...
                    <td>
                        <button type="button" class="icon-btn"
                            ...
                            onclick="window.location.href='?page=produkt-durations&category=<?php echo $selected_category; ?>&tab=edit&edit=<?php echo $duration->id; ?>'">
                        ...
                        <button type="button" class="icon-btn"
                            onclick="if(confirm('<?php echo esc_js(__('Bist du sicher, dass du löschen möchtest?', 'h2-rental-pro')); ?>')){window.location.href='?page=produkt-durations&category=<?php echo $selected_category; ?>&delete=<?php echo $duration->id; ?>&fw_nonce=<?php echo wp_create_nonce('produkt_admin_action'); ?>';}"
