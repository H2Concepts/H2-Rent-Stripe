...
class Database {
    ...
    public static function backfill_review_targets_batch($limit = 200) {
        ...
        $orders = $wpdb->get_results(
            $wpdb->prepare(
                "SELECT id, customer_email, category_id, order_items, end_date, stripe_subscription_id, mode, status
    ...
    public static function get_review_admin_metrics_fast() {
        ...
        $cache_key = 'produkt_reviews_admin_metrics_v3';
        ...
        // Zähle alle aktiven Mietabos (alle Bestellungen die nicht Kauf sind)
        // Berechne direkt aus den Bestellungen
        $orders_table = $wpdb->prefix . 'produkt_orders';
        
        // Zähle alle Mietabos (nicht Kauf-Bestellungen)
        $total_rentals = (int) $wpdb->get_var(
            "SELECT COUNT(DISTINCT o.id)
             FROM $orders_table o
             WHERE (o.mode IS NULL OR (o.mode != 'kauf' AND o.mode != 'sale'))"
        );
        
        // Zähle wie viele Bestellungen bereits bewertet wurden
        // Prüfe direkt in der reviews Tabelle, welche subscription_keys bewertet wurden
        // und zähle dann, wie viele Bestellungen mindestens eine Bewertung haben
        $reviewed_order_ids = [];
        
        if ($total_rentals > 0) {
            // Hole alle Bewertungen mit ihren subscription_keys
            $all_reviews = $wpdb->get_results(
                "SELECT DISTINCT r.subscription_key, c.email
                 FROM $reviews_table r
                 LEFT JOIN $customers_table c ON r.customer_id = c.id
                 WHERE r.status = 'approved'"
            );
            
            // Erstelle eine Map von subscription_key zu customer_email
            $reviewed_keys_map = [];
            foreach ($all_reviews as $review) {
                if (!empty($review->subscription_key) && !empty($review->email)) {
                    $reviewed_keys_map[$review->subscription_key] = $review->email;
                }
            }
            
            // Hole alle Mietabos
            $all_rentals = $wpdb->get_results(
                "SELECT o.id, o.customer_email, o.stripe_subscription_id, o.order_items
                 FROM $orders_table o
                 WHERE (o.mode IS NULL OR (o.mode != 'kauf' AND o.mode != 'sale'))"
            );
            
            foreach ($all_rentals as $order) {
                // Erstelle mögliche subscription_keys für diese Bestellung
                $possible_keys = [];
                
                if (!empty($order->stripe_subscription_id)) {
                    $base_key = trim((string) $order->stripe_subscription_id);
                    if (!empty($order->order_items)) {
                        require_once PRODUKT_PLUGIN_PATH . 'includes/account-helpers.php';
                        $items = pv_expand_order_products((object) $order);
                        foreach ($items as $idx => $item) {
                            $possible_keys[] = $base_key . '-' . $idx;
                        }
                    } else {
                        $possible_keys[] = $base_key . '-0';
                        // Prüfe auch weitere Indizes
                        for ($i = 1; $i <= 10; $i++) {
                            $possible_keys[] = $base_key . '-' . $i;
                        }
                    }
                }
                
                $possible_keys[] = 'order-' . $order->id . '-0';
                
                // Prüfe ob eine dieser Keys bewertet wurde (und vom gleichen Kunden)
                foreach ($possible_keys as $key) {
                    if (isset($reviewed_keys_map[$key]) && $reviewed_keys_map[$key] === $order->customer_email) {
                        $reviewed_order_ids[$order->id] = true;
                        break; // Eine Bewertung reicht, um die Bestellung als bewertet zu markieren
                    }
                }
            }
        }
        
        $reviewed_rentals = count($reviewed_order_ids);
        $unreviewed = max(0, $total_rentals - $reviewed_rentals);
